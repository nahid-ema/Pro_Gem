    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 animate-fadeIn">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-xl relative">
        ...
        <div className="text-center mb-6">
          <div className="w-12 h-12 rounded-2xl bg-[#4F46E5] text-white flex items-center justify-center font-black text-xl mx-auto mb-3 shadow-none">
            🏠
          </div>
            ...
            <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-full mb-5 text-xs font-semibold">
              <button
                ...
                className={`flex-1 py-1.5 rounded-sm transition-all cursor-pointer ${
                  mode === 'signIn'
                    ? 'bg-[#F5F5F0] dark:bg-[#1A1A1A]/80 text-[#4F46E5] shadow-none font-bold'
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              <div>
                ...
                <div className="relative">
                  <i className="fi fi-sr-envelope text-base absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
                  <input
                    type="email"
                    required
                    value={email}
                    ...
                    className="w-full pl-10 pr-3 py-2 rounded-sm border border-[#D6D0C4] dark:border-slate-800 bg-[#EBE7E0] dark:bg-slate-800 text-xs md:text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#4F46E5]"
                  />
                </div>
              </div>
              ...
              {mode !== 'resetPassword' && (
                <div>
                  ...
                  <div className="relative">
                    <i className="fi fi-sr-key text-base absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
                    <input
                      type="password"
                      required
                      value={password}
              ...
              {statusMsg && (
                <div className={`p-3 rounded-sm text-xs font-semibold flex items-center gap-2 ${
                  ...
                  {isError ? <i className="fi fi-sr-info text-base shrink-0" /> : <i className="fi fi-sr-check-circle text-base shrink-0" />}
                  <span>{statusMsg}</span>
                </div>
              )}
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-2.5 rounded-sm bg-[#4F46E5] text-white hover:bg-[#4338CA] font-bold text-xs md:text-sm shadow-none transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
              >
                {mode === 'signIn' && <i className="fi fi-sr-sign-in-alt" />}
                {mode === 'signUp' && <i className="fi fi-sr-user-add" />}
                {mode === 'resetPassword' && <i className="fi fi-sr-refresh text-base" />}
                <span>
                  {isLoading
